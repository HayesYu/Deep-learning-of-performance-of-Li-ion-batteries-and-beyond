...
def extract_features_from_cif(cif_path):
    ...
    try:
        ...
        # 特别关注镁含量（对镁电池材料特别重要）
        mg_fraction = element_fractions.get('Mg', 0)
...
def main(input_csv, cif_directory, output_csv):
    ...
    # 8. Save model
    with open('KRR_Mg_capacity_model.pkl', 'wb') as f:  # 改为Mg模型名称
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KRR model for Mg-ion battery material prediction")
