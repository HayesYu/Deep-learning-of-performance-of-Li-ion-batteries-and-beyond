...
def extract_features_from_cif(cif_path):
    ...
    try:
        ...
        # 特别关注钠含量（对钠电池材料特别重要）
        na_fraction = element_fractions.get('Na', 0)
...
def main(input_csv, cif_directory, output_csv):
    ...
    # 8. Save model
    with open('KRR_Na_capacity_model.pkl', 'wb') as f:  # 改为Na模型名称
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KRR model for Na-ion battery material prediction")
