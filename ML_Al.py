...
def extract_features_from_cif(cif_path):
    ...
    try:
        ...
        # 特别关注铝含量（对铝离子电池材料特别重要）
        al_fraction = element_fractions.get('Al', 0)
...
def main(input_csv, cif_directory, output_csv):
    ...
    # 8. Save model
    with open('KRR_Al_capacity_model.pkl', 'wb') as f:  # 改为Al模型名称
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KRR model for Al-ion battery material prediction")
    parser.add_argument("--input_csv", type=str, default="/home/user1/code/data/material_Al/id_prop.csv",
                        help="Input CSV file with IDs and target values")
    parser.add_argument("--cif_dir", type=str, default="/home/user1/code/data/material_Al/",
