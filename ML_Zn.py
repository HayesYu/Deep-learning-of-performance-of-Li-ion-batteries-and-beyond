...
def extract_features_from_cif(cif_path):
    ...
    try:
        ...
        # 电池材料常见元素的分数 - 添加Zn
        common_elements = ['Li', 'Na', 'K', 'Mg', 'Ca', 'Al', 'Zn',  # 阳离子
                          'O', 'F', 'S', 'P', 'N',  # 阴离子
        ...
        # 特别关注锌含量（对锌离子电池材料特别重要）
        zn_fraction = element_fractions.get('Zn', 0)
        
        # 阴离子/阳离子比例（影响离子导电性）
        anions = ['O', 'F', 'S', 'P', 'N']
        cations = ['Li', 'Na', 'K', 'Mg', 'Ca', 'Al', 'Zn', 'Co', 'Ni', 'Mn', 'Fe', 'V', 'Ti', 'Cr', 'Te']
...
def main(input_csv, cif_directory, output_csv):
    ...
    # 8. Save model
    with open('KRR_Zn_capacity_model.pkl', 'wb') as f:  # 改为Zn模型名称
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KRR model for Zn-ion battery material prediction")
    parser.add_argument("--input_csv", type=str, default="/home/user1/code/data/material_Na/id_prop.csv",
                        help="Input CSV file with IDs and target values")
    parser.add_argument("--cif_dir", type=str, default="/home/user1/code/data/material_Na/",
